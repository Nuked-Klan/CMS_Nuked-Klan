...
if (_NKCAPTCHA == "off")
     $captcha = 0;
else if ((_NKCAPTCHA == 'auto' OR _NKCAPTCHA == 'on') && $user[1] > 0)
     $captcha = 0;
else
     $captcha = 1;
...
if ($visiteur >= $level_access && $level_access > -1) {
     ...
     function edit($mess_id) {
          ...
          if ($_REQUEST['titre'] == "" || $_REQUEST['texte'] == "" || @ctype_space($_REQUEST['titre']) || @ctype_space($_REQUEST['texte'])) {
               ...
               $url = "index.php?file=Forum&page=post&forum_id=" . (int) $_REQUEST['forum_id'] . "&mess_id=" . $_REQUEST['mess_id'] . "&do=edit";
               redirect($url, 2);
               closetable();
               footer();
               exit();
          ...
          $result = mysql_query("SELECT moderateurs FROM " . FORUM_TABLE . " WHERE '" . $visiteur . "' >= level AND id = '" . (int) $_REQUEST['forum_id'] . "'");
          ...
          if ($_REQUEST['author'] == $user[2] || $visiteur >= admin_mod("Forum") || $administrator == 1) {
               ...
               if (!is_numeric($_REQUEST['usersig']))
                    $_REQUEST['usersig'] = 0;
               if (!is_numeric($_REQUEST['emailnotify']))
                    $_REQUEST['emailnotify'] = 0;
               ...
               mysql_query("UPDATE " . FORUM_MESSAGES_TABLE . " SET titre = '" . $_REQUEST['titre'] . "', txt = '" . $_REQUEST['texte'] . "'" . $edition . ", usersig = '" . $_REQUEST['usersig'] . "', emailnotify = '" . $_REQUEST['emailnotify'] . "' WHERE id = '" . $mess_id . "'");

               if ($mid == $mess_id) {
                    mysql_query("UPDATE " . FORUM_THREADS_TABLE . " SET titre = '" . $_REQUEST['titre'] . "' WHERE id = '" . $thread_id . "'");
               }
               ...
               if ($nb_rep > $nuked['mess_forum_page']) {
                    ...
                    $url = "index.php?file=Forum&page=viewtopic&forum_id=" . (int) $_REQUEST['forum_id'] . "&thread_id=" . $thread_id . "&p=" . $topicpages . "#" . $mess_id;
               } else {
                    $url = "index.php?file=Forum&page=viewtopic&forum_id=" . (int) $_REQUEST['forum_id'] . "&thread_id=" . $thread_id . "#" . $mess_id;
               }
     ...
     function del($mess_id) {
          ...
          $result = mysql_query("SELECT moderateurs FROM " . FORUM_TABLE . " WHERE '" . $visiteur . "' >= niveau AND id = '" . (int) $_REQUEST['forum_id'] . "'");
          list($modos) = mysql_fetch_array($result);
          ...
          if ($visiteur >= admin_mod("Forum") || $administrator == 1) {
               if ($_REQUEST['confirm'] == _YES) {
                    $sql2 = mysql_query("SELECT id, file FROM " . FORUM_MESSAGES_TABLE . " WHERE thread_id = '" . (int) $_REQUEST['thread_id'] . "' ORDER BY id LIMIT 0, 1");
                    ...
                    if ($mid == $mess_id) {
                         $sql_survey = mysql_query("SELECT sondage FROM " . FORUM_THREADS_TABLE . " WHERE id = '" . (int) $_REQUEST['thread_id'] . "'");
                         list($sondage) = mysql_fetch_row($sql_survey);

                         if ($sondage == 1) {
                              $sql_poll = mysql_query("SELECT id FROM " . FORUM_POLL_TABLE . " WHERE thread_id = '" . (int) $_REQUEST['thread_id'] . "'");
                              list($poll_id) = mysql_fetch_row($sql_poll);

                              mysql_query("DELETE FROM " . FORUM_POLL_TABLE . " WHERE id = '" . $poll_id . "'");
                              mysql_query("DELETE FROM " . FORUM_OPTIONS_TABLE . " WHERE poll_id = '" . $poll_id . "'");
                              mysql_query("DELETE FROM " . FORUM_VOTE_TABLE . " WHERE poll_id = '" . $poll_id . "'");
                         }
                    ...
                    mysql_query("DELETE FROM " . FORUM_MESSAGES_TABLE . " WHERE id = '" . $mess_id . "'");
                    ...
                    . "<input type=\"hidden\" name=\"forum_id\" value=\"" . (int) $_REQUEST['forum_id'] . "\" />\n"
                    . "<input type=\"hidden\" name=\"thread_id\" value=\"" . (int) $_REQUEST['thread_id'] . "\" />\n"
     ...
     function del_topic($thread_id) {
          ...
          if ($visiteur >= admin_mod("Forum") || $administrator == 1) {
               if ($_REQUEST['confirm'] == _YES) {
                    ...
                    $url = "index.php?file=Forum&page=viewforum&forum_id=" . (int) $_POST['forum_id'];
                    echo "<br /><br /><div style=\"text-align: center;\">" . _TOPICDELETED . "</div><br /><br />";
                    redirect($url, 2);
               } else if ($_REQUEST['confirm'] == _NO) {
                    $url = "index.php?file=Forum&page=viewtopic&forum_id=" . (int) $_REQUEST['forum_id'] . "&thread_id=" . $thread_id;
                    echo "<br /><br /><div style=\"text-align: center;\">" . _DELCANCEL . "</div><br /><br />";
                    redirect($url, 2);
               } else {
     ...
     function move() {
          ...
          if ($visiteur >= admin_mod("Forum") || $administrator == 1) {
               if ($_REQUEST['confirm'] == _YES && $_REQUEST['newforum'] != "") {
                    ...
                    . "<input type=\"hidden\" name=\"forum_id\" value=\"" . (int) $_REQUEST['forum_id'] . "\" />\n"
                    . "<input type=\"hidden\" name=\"thread_id\" value=\"" . (int) $_REQUEST['thread_id'] . "\" /></div></form><br />\n";
               }
          } else {
               echo "<br /><br /><div style=\"text-align: center;\">" . _ZONEADMIN . "</div><br /><br />";
               $url = "index.php?file=Forum&page=viewtopic&forum_id=" . (int) $_REQUEST['forum_id'] . "&thread_id=" . (int) $_REQUEST['thread_id'];
     ...
     function lock() {
          ...
          if ($visiteur >= admin_mod("Forum") || $administrator == 1) {
               ...
               mysql_query("UPDATE " . FORUM_THREADS_TABLE . " SET closed = '" . $lock_type . "' WHERE id = '" . (int) $_REQUEST['thread_id'] . "'");
     ...
     function announce() {
          ...
          if ($visiteur >= admin_mod("Forum") || $administrator == 1) {
               ...
               mysql_query("UPDATE " . FORUM_THREADS_TABLE . " SET annonce = '" . $announce . "' WHERE id = '" . (int) $_REQUEST['thread_id'] . "'");
     ...
     function reply() {
          ...
          if ($auth == "0") {
               ...
               $url = "index.php?file=Forum&page=post&forum_id=" . (int) $_REQUEST['forum_id'] . "&thread_id=" . (int) $_REQUEST['thread_id'];
               redirect($url, 2);
               closetable();
               footer();
               exit();
          ...
          if ($nbusers > 0) {
               while (list($usermail) = mysql_fetch_row($notify)) {
                    if ($usermail != $auteur_id) {
                         ...
                         $corps = _EMAILNOTIFYMAIL . "\r\n" . $nuked['url'] . "/index.php?file=Forum&page=viewtopic&forum_id=" . (int) $_REQUEST['forum_id'] . "&thread_id=" . (int) $_REQUEST['thread_id'] . "\r\n\r\n\r\n" . $nuked['name'] . " - " . $nuked['slogan'];
          ...
          if ($user) {
               ...
               $newcount = $count + 1;
               mysql_query("UPDATE " . USER_TABLE . " SET count = '" . $newcount . "' WHERE id = '" . $user[0] . "'");
          }
          ...
          if ($nb_rep > $nuked['mess_forum_page']) {
               $topicpages = $nb_rep / $nuked['mess_forum_page'];
               $topicpages = ceil($topicpages);
               $link_post = "index.php?file=Forum&page=viewtopic&forum_id=" . (int) $_REQUEST['forum_id'] . "&thread_id=" . (int) $_REQUEST['thread_id'] . "&p=" . $topicpages . "#" . $mess_id;
          } else {
               $link_post = "index.php?file=Forum&page=viewtopic&forum_id=" . (int) $_REQUEST['forum_id'] . "&thread_id=" . (int) $_REQUEST['thread_id'] . "#" . $mess_id;
          }
     ...
     function post() {
          global $user, $nuked, $captcha, $user_ip, $visiteur, $bgcolor3, $administrator;
          ...
          if ($_REQUEST['auteur'] == "" || $_REQUEST['titre'] == "" || $_REQUEST['texte'] == "" || @ctype_space($_REQUEST['titre']) || @ctype_space($_REQUEST['texte'])) {
               echo "<br /><br /><div style=\"text-align: center;\">" . _FIELDEMPTY . "</div><br /><br />";
               $url = "index.php?file=Forum&page=post&forum_id=" . (int) $_REQUEST['forum_id'];
               redirect($url, 2);
               closetable();
               footer();
               exit();
          ...
          if (!is_numeric($_REQUEST['emailnotify']))
               $_REQUEST['emailnotify'] = 0;
          if (($visiteur < admin_mod("Forum") && $administrator == 0) || !is_numeric($_REQUEST['annonce']))
               $_REQUEST['annonce'] = 0;
          ...
          mysql_query("INSERT INTO " . FORUM_THREADS_TABLE . " ( `id` , `titre` , `date` , `closed` , `auteur` , `auteur_id` , `forum_id` , `last_post` , `view` , `annonce` , `sondage` ) VALUES ( '' , '" . $_REQUEST['titre'] . "' , '" . $date . "' , '' , '" . $autor . "' , '" . $auteur_id . "' , '" . (int) $_REQUEST['forum_id'] . "' , '" . $date . "' , '' , '" . $_REQUEST['annonce'] . "' , '" . $sondage . "' )");
          $req4 = mysql_query("SELECT MAX(id) FROM " . FORUM_THREADS_TABLE . " WHERE forum_id = '" . (int) $_REQUEST['forum_id'] . "' AND titre = '" . $_REQUEST['titre'] . "' AND date = '" . $date . "' AND auteur = '" . $_REQUEST['auteur'] . "'");
          $idmax = mysql_result($req4, 0, "MAX(id)");

          (int) $_REQUEST['thread_id'] = $idmax;
          ...
          if ($_REQUEST['survey'] == 1 && $_REQUEST['survey_field'] > 0 && $visiteur >= $level_poll) {
               $url = "index.php?file=Forum&op=add_poll&survey_field=" . $_REQUEST['survey_field'] . "&forum_id=" . (int) $_REQUEST['forum_id'] . "&thread_id=" . (int) $_REQUEST['thread_id'];
          } else {
               $url = "index.php?file=Forum&page=viewtopic&forum_id=" . (int) $_REQUEST['forum_id'] . "&thread_id=" . (int) $_REQUEST['thread_id'];
          }
          ...
          if ($user) {
               if ((int) $_REQUEST['forum_id'] > 0) {
                    ...
                    if (isset($_COOKIE[$cookie_forum]) && $_COOKIE[$cookie_forum] != "") {
                         ...
                         if (preg_match("`[^0-9,]`i", $id_read_forum))
                              $id_read_forum = "";
                         $table_read_forum = explode(',', $id_read_forum);
                    }

                    $req = "SELECT MAX(id) FROM " . FORUM_MESSAGES_TABLE . " WHERE forum_id = '" . (int) $_REQUEST['forum_id'] . "' AND date > '" . $user[4] . "' GROUP BY thread_id";
                    $sql = mysql_query($req);
     ...
     function del_file() {
          ...
          if ($user && $auteur_id == $user[0] || $visiteur >= admin_mod("Forum") || $administrator == 1) {
               ...
               if (is_file($path)) {
                    ...
                    mysql_query("UPDATE " . FORUM_MESSAGES_TABLE . " SET file = '' WHERE id = '" . $_REQUEST['mess_id'] . "'");
     ...
     function add_poll() {
          ...
          if ($user && $user[0] == $auteur_id && $sondage == 1 && $visiteur >= $level_poll) {
               ...
               echo "<tr><td>&nbsp;<input type=\"hidden\" name=\"thread_id\" value=\"" . (int) $_REQUEST['thread_id'] . "\" />\n"
               . "<input type=\"hidden\" name=\"forum_id\" value=\"" . (int) $_REQUEST['forum_id'] . "\" />\n"
     ...
     function send_poll($titre, $option, $thread_id, $forum_id, $max_option) {
          ...
          if ($user && $user[0] == $auteur_id && $sondage == 1 && $visiteur >= $level_poll) {
               if ($option[1] != "") {
                    ...
                    mysql_query("INSERT INTO " . FORUM_POLL_TABLE . " ( `id` , `thread_id` , `titre` ) VALUES ( '' , '" . $thread_id . "' , '" . $titre . "' )");
                    ...
                    while ($r < $max) {
                         ...
                         if ($options != "") {
                              mysql_query("INSERT INTO " . FORUM_OPTIONS_TABLE . " ( `id` , `poll_id` , `option_text` , `option_vote` ) VALUES ( '" . $vid . "' , '" . $poll_id . "' , '" . $options . "' , '' )");
                         }
                         $r++;
                    }
          ...
          if ($_REQUEST['voteid'] != "") {
               if ($visiteur > 0) {
                    ...
                    if ($visiteur >= $level_vote) {
                         ...
                         if ($check == 0) {
                              mysql_query("UPDATE " . FORUM_OPTIONS_TABLE . " SET option_vote = option_vote + 1 WHERE id = '" . $_REQUEST['voteid'] . "' AND poll_id = '" . $poll_id . "'");
                              mysql_query("INSERT INTO " . FORUM_VOTE_TABLE . " ( `poll_id` , `auteur_id` , `auteur_ip` ) VALUES ( '" . $poll_id . "' , '" . $user[0] . "' , '" . $user_ip . "' )");
     ...
     function del_poll($poll_id, $thread_id, $forum_id) {
          ...
          if ($user && $user[0] == $auteur_id || $visiteur >= admin_mod("Forum") || $administrator == 1) {
               if ($_REQUEST['confirm'] == _YES) {
                    ...
                    mysql_query("DELETE FROM " . FORUM_OPTIONS_TABLE . " WHERE poll_id = '" . $poll_id . "'");
                    mysql_query("DELETE FROM " . FORUM_VOTE_TABLE . " WHERE poll_id = '" . $poll_id . "'");
                    mysql_query("UPDATE " . FORUM_THREADS_TABLE . " SET sondage = 0 WHERE id = '" . $thread_id . "'");
     ...
     function edit_poll($poll_id) {
          ...
          if ($user && $user[0] == $auteur_id || $visiteur >= admin_mod("Forum") || $administrator == 1) {
               ...
               . "<input type=\"hidden\" name=\"thread_id\" value=\"" . (int) $_REQUEST['thread_id'] . "\" />\n"
               . "<input type=\"hidden\" name=\"forum_id\" value=\"" . (int) $_REQUEST['forum_id'] . "\" /></td></tr>\n"
     ...
     function modif_poll($poll_id, $titre, $option, $newoption, $thread_id, $forum_id) {
          ...
          if ($user && $user[0] == $auteur_id || $visiteur >= admin_mod("Forum") || $administrator == 1) {
               ...
               mysql_query("UPDATE " . FORUM_POLL_TABLE . " SET titre = '" . $titre . "' WHERE id = '" . $poll_id . "'");
               ...
               while ($r < $nuked['forum_field_max']) {
                    ...
                    if ($options != "") {
                         mysql_query("UPDATE " . FORUM_OPTIONS_TABLE . " SET option_text = '" . $options . "' WHERE poll_id = '" . $poll_id . "' AND id = '" . $r . "'");
                    } else {
                         mysql_query("DELETE FROM " . FORUM_OPTIONS_TABLE . " WHERE poll_id = '" . $poll_id . "' AND id = '" . $r . "'");
                    }
               }
               ...
               if ($newoption != "") {
                    ...
                    mysql_query("INSERT INTO " . FORUM_OPTIONS_TABLE . " ( `id` , `poll_id` , `option_text` , `option_vote` ) VALUES ( '" . $s . "' , '" . $poll_id . "' , '" . $newoption . "', '0')");
               }
     ...
     function notify() {
          ...
          if ($user[0] != "") {
               ...
               mysql_query("UPDATE " . FORUM_MESSAGES_TABLE . " SET emailnotify = '" . $notify . "' WHERE thread_id = '" . (int) $_REQUEST['thread_id'] . "' AND auteur_id = '" . $user[0] . "'");
     ...
     switch ($_REQUEST['op']) {
          ...
          case"del_topic":
               del_topic((int) $_REQUEST['thread_id']);
               break;
          ...
          case"send_poll":
               send_poll($_REQUEST['titre'], $_REQUEST['option'], (int) $_REQUEST['thread_id'], (int) $_REQUEST['forum_id'], $_REQUEST['max_option']);
               break;
          ...
          case"del_poll":
               del_poll($_REQUEST['poll_id'], (int) $_REQUEST['thread_id'], (int) $_REQUEST['forum_id']);
               break;
          ...
          case"modif_poll":
               modif_poll($_REQUEST['poll_id'], $_REQUEST['titre'], $_REQUEST['option'], $_REQUEST['newoption'], (int) $_REQUEST['thread_id'], (int) $_REQUEST['forum_id']);
               break;
