...
if ($visiteur >= $level_access && $level_access > -1)
{
    ...
    $topic_ok = mysql_num_rows($sql2);
     ...
     // User access
          ...
          if ($user) {
               ...
               $visit = mysql_query("SELECT user_id, thread_id, forum_id FROM " . FORUM_READ_TABLE . " WHERE user_id = '" . $GLOBALS['user']['id'] . "'") or die(mysql_error());
               ...
               if (!$user_visit || strrpos($user_visit['thread_id'], ',' . $_GET['thread_id'] . ',') === false || strrpos($user_visit['forum_id'], ',' . $_GET['forum_id'] . ',') === false) {
                    ...
                    mysql_query("REPLACE INTO " . FORUM_READ_TABLE . " ( `user_id` , `thread_id` , `forum_id` ) VALUES ( '" . $GLOBALS['user']['id'] . "' , '," . $tid . "' , '," . $fid . "' )") or die(mysql_error());
            }
        }
    ...
    . "<table style=\"background: " . $color3 . ";\" width=\"100%\" cellspacing=\"1\" cellpadding=\"4\" border=\"0\" id=\"img_resize_forum\">\n";
        ...
        if ($sondage == 1)
        {
            ...
            if ($user && $topic_aid == $GLOBALS['user']['id'] && $closed == 0 || $visiteur >= admin_mod("Forum") || $administrator == 1)
            {
            ...
            $check = mysql_query("SELECT auteur_ip FROM " . FORUM_VOTE_TABLE . " WHERE poll_id = '" . $poll_id . "' AND auteur_id = '" . $GLOBALS['user']['id'] . "'");
        ...
        while (list($mess_id, $title, $auteur, $auteur_id, $auteur_ip, $txt, $date, $edition, $usersig, $fichier) = mysql_fetch_row($sql4))
        {
            ...
            if ($user && $auteur_id == $GLOBALS['user']['id'] && $closed == 0 || $visiteur >= admin_mod("Forum") || $administrator == 1)
            {
        ...
        if ($GLOBALS['user']['id'] != "")
        {
            $sql_notify = mysql_query("SELECT emailnotify FROM " . FORUM_MESSAGES_TABLE . " WHERE thread_id = '" . $_REQUEST['thread_id'] . "' AND auteur_id = '" . $GLOBALS['user']['id'] . "'");
