...
global $nuked, $user, $language, $theme, $color1, $color2, $color3, $background;
...
if ($visiteur >= $level_access && $level_access > -1) {
     ...
     // User access
          ...
          mysql_query("UPDATE " . FORUM_THREADS_TABLE . " SET view = view + 1 WHERE forum_id = '" . $_REQUEST['forum_id'] . "' AND id = '" . $_REQUEST['thread_id'] . "'");
          ...
          while (list($mess_id, $title, $auteur, $auteur_id, $auteur_ip, $txt, $date, $edition, $usersig, $fichier) = mysql_fetch_row($sql4)) {
                    ...
                    for ($i = 0; $i < count($search); $i++) {
                         ...
                         foreach ($tab as $key => $val) {
                              if (preg_match("`^<\w+`", $val))
                                   ...
                                   $tab[$key] = preg_replace("/$search[$i]/", "<span style=\"color: #FF0000;\"><b>$0</b></span>", $val);
                         }
               ...
               if (strftime("%d %m %Y", time()) == strftime("%d %m %Y", $date))
                    $date = _FTODAY . "&nbsp;" . strftime("%H:%M", $date);
               else if (strftime("%d", $date) == (strftime("%d", time()) - 1) && strftime("%m %Y", time()) == strftime("%m %Y", $date))
                    $date = _FYESTERDAY . "&nbsp;" . strftime("%H:%M", $date);
               else
                    $date = _THE . ' ' . nkDate($date);
                    ...
                    if ($test > 0 && $autor != "") {
                         ...
                         if ($avatar != "") {
                              if ($avatar_resize == "off")
                                   $ar_ok = 0;
                              else if (preg_match("`http://`i", $avatar) && $avatar_resize == "local")
                                   $ar_ok = 0;
                              else
                                   $ar_ok = 1;
