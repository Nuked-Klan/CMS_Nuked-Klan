...
if ($visiteur >= $level_access && $level_access > -1) {
     ...
     if ($level_ok == 0) {
          ...
          while (list($thread_id, $titre, $auteur, $auteur_id, $nb_read, $closed, $annonce, $sondage) = mysql_fetch_row($sql3)) {
               ...
               if ($auteur_id != "") {
                    $sql5 = mysql_query("SELECT pseudo FROM " . USER_TABLE . " WHERE id = '" . $auteur_id . "'");
                    $test = mysql_num_rows($sql5);
                    list($autor) = mysql_fetch_array($sql5);
               ...
               if (strftime("%d %m %Y", time()) == strftime("%d %m %Y", $last_date))
                    $last_date = _FTODAY . "&nbsp;" . strftime("%H:%M", $last_date);
               else if (strftime("%d", $last_date) == (strftime("%d", time()) - 1) && strftime("%m %Y", time()) == strftime("%m %Y", $last_date))
                    $last_date = _FYESTERDAY . "&nbsp;" . strftime("%H:%M", $last_date);
               else
               ...
               if ($last_auteur_id != "") {
                    $sql8 = mysql_query("SELECT pseudo FROM " . USER_TABLE . " WHERE id = '" . $last_auteur_id . "'");
                    $test1 = mysql_num_rows($sql8);
                    list($last_autor) = mysql_fetch_array($sql8);
