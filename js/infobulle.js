...
function AffBulle(title, text, width){
	...
	content = '<table style="border: ' + Params.Border.Width + 'px ' + Params.Border.Style + ' ' + Params.Color + '; -moz-border-radius: ' + radius + 'px;" cellspacing="' + spacing + '" cellpadding="' + padding + '">';

	if(t){
		content +='	<tr>'
				+ '		<td style="background: ' + Params.Background + ';">&nbsp;&raquo;&nbsp;&nbsp;<b>' + title + '</b>&nbsp;</td>'
				+ '	</tr>';
	}

	content +='	<tr>'
			+ '		<td style=\"padding: 0px; margin: auto; height: 1px; background-color: ' + Params.Color + ';\"></td>'
			+ '	</tr>'
			+ '	<tr>'
			+ '		<td style="background: ' + Params.Background + '; vertical-align: top">'
			+ '			<table width="100%" style="background: ' + Params.Background + ';" cellpadding="' + padding + '" cellspacing="' + spacing + '">'
			+ '				<tr>'
			+ '					<td>' + text + '</td>'
			+ '				</tr>'
			+ '			</table>'
...
function InitBulle(background, color, params){
	...
	Params.Color = color;
	Params.Opacity = 85;	
	Params.Border = new Object();	
	Params.Border.Width = 1;
	Params.Border.Style = 'solid';
	Params.Border.Color = '#232323';
