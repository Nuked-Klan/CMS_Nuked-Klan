...
$sql = nkDB_execute('SELECT FTT.id, FTT.titre, FTT.last_post, FTT.forum_id FROM ' . FORUM_THREADS_TABLE . ' AS FTT INNER JOIN ' . FORUM_TABLE . ' AS FT ON FT.id = FTT.forum_id WHERE FT.niveau <= ' . $visiteur . ' ORDER BY last_post DESC LIMIT 0, '.$this->get('blockForumNbTopics'));
while (list($thread_id, $titre, $last_post, $forum_id) = nkDB_fetchRow($sql)) {
			  
	$titre = printSecuTags($titre);
	$date = nkDate($last_post);

    $sql2 = nkDB_execute("SELECT id, auteur FROM " . FORUM_MESSAGES_TABLE . " WHERE thread_id = '" . $thread_id . "' ORDER BY id DESC LIMIT 0, 1");
    list($mess_id, $auteur) = nkDB_fetchArray($sql2);
