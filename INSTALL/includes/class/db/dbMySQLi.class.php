...
class dbMySQLi {
    ...
    /*
     * Store MySQLi username
     */
    private $_user = '';

    /*
     * Store MySQLi password
     */
    private $_pass = '';

    /*
     * Store MySQLi database name
     */
    private $_dbName = '';

    /*
     * Store MySQLi link identifier
     */
    private $_db;

    /*
     * Constructor - Check and store MySQLi connection data
     */
    ...
    public function connect() {
        if (($this->_db = @mysqli_connect($this->_host, $this->_user, $this->_pass, $this->_dbName)) !== false) {
            @mysqli_query($this->_db, 'SET NAMES '. db::CHARSET);
    ...
    public function execute($sql, $options = array()) {
        ...
        if (($result = @mysqli_query($this->_db, $sql)) === false) {
    ...
    public function selectOne($sql, $options = array()) {
        ...
        if (in_array(DB_NUM, $options))
            $mysqlFetchCmd = 'mysqli_fetch_row';
        else
            $mysqlFetchCmd = 'mysqli_fetch_assoc';
     ...
     */
    public function getForeignKeyList($table, $options = array()) {
    ...
    public function tableExist($table, $options = array()) {
        ...
        return (mysqli_num_rows($dbsTable) == 0) ? false : true;
    }
