...
function getStats($nuked)
{
			...
			$data['server_ip'] = (strlen($nuked['server_ip'])>0) ? "1" : "0";                                                     // Verifier si utile
			$data['server_game'] = ($server_ip == "set" && strlen($nuked['server_game'])>0) ? $nuked['server_game'] : "0";          // Verifier si utile
