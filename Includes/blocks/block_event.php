...
function affich_block_event($blok)
{
    ...
    foreach($days as $day)
    {
        $blok['content'] .= "<td align=\"center\"><b>" . $day . "</b></td>";
    }
    ...
    for($count = 0;$count < (6 * 7);$count++)
    {
        ...
        else
        {
            ...
            if ($nb_match > 0 || $nb_event > 0 || $nb_birthday > 0)
            {
                    ...
                    if (defined("WARS_TABLE"))
                    {
                        ...
                        while (list($etat, $adv_name, $type_match, $jour, $mois, $an, $heure, $style, $score_team, $score_adv) = mysql_fetch_array($sql))
                        {
                            if ($etat == 1)
                            {
                                ...
                                {
                                    $scores = _RESULT . " : <span style=\"color: #990000\"><b>" . $score_team . " - " . $score_adv . "</b></span>";
                                }
                                else if ($score_team > $score_adv)
                                {
                                    $scores = _RESULT . " : <span style=\"color: #009900\"><b>" . $score_team . " - " . $score_adv . "</b></span>";
                                }
                                else
                                {
                ...
                $blok['content'] .= "<td style=\"background: " . $bgcolor1 . ";border: 1px solid " . $bgcolor3 . ";\" align=\"center\" onmouseover=\"AffBulle('&nbsp;&nbsp;&nbsp;&nbsp;" . $event_date . "&nbsp;" . $months[$this_month] . "&nbsp;" . $year . "', '" . htmlentities(mysql_real_escape_string($txt), ENT_NOQUOTES) . "', 200)\" onmouseout=\"HideBulle()\">\n"
